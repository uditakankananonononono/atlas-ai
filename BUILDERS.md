...
- pb8 (branch `pb8`, 2026-09-24): M16 read-only card for M04 scheduled re-runs. Files: `m16_executive_dashboard/rerun_card.py`, `frontend/components/executive-dashboard/RerunScheduleCard.tsx`; additive lines in m16 `routes.py`/`schemas.py`/`service.py` (widget kind + default/appended widget), `frontend/components/ExecutiveDashboard.tsx`, `executive-dashboard/api.ts`.
