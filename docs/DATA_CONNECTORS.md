...
Research/news/venture discovery uses free public sources: arXiv Atom, bioRxiv/medRxiv API, PubMed E-utilities, Semantic Scholar Academic Graph, generic RSS/Atom, Hacker News Algolia, and Product Hunt's public feed. Every normalized item keeps its original URL/payload and the seed-interest topic that matched it. The shared tenant-scoped storage, content-hash dedupe, and run/request/cost logs apply unchanged. Optional free API keys increase provider quotas but are not required for the public routes.
