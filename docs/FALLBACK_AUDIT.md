...
| `m01_opportunity_discovery/service.py` keyword type tagger | disguised stub | No live spaCy/entity or trained classification path in the M1 scan | build real optional spaCy/dateparser/classifier pipeline with provenance |
| `m01_opportunity_discovery/service.py` token cosine | disguised stub | Requested embedding matching is replaced by token counts despite a shared embedding provider existing elsewhere | wire shared embedding provider/vector store |
