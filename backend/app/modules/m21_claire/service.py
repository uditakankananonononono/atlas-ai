...
class Service:
 FORBIDDEN=("self-bot","oceanofpdf","pirated","fabricate credentials","rotating proxy","login scrape","disable approval")
 ...
 def intake(self,goal:str,acceptance:list[str],limits:dict[str,Any]):
  ...
  item=ClaireGoal(goal,acceptance,{"environment":"paired_local_pc","max_retries":self.max_retries,**limits});self.goals[item.id]=item;return item
 ...
 def request_environment_change(self,goal_id:str,operation:str,preview:dict[str,Any]):
  if operation not in {"install_package","write_file","move_file","delete_file","type_text","click","browser_navigate","run_command","deploy_preview","connect_tool"}:raise ValueError("local operation not supported")
 ...
 async def local_action(self,goal_id:str,action:dict[str,Any],approval_token:str|None=None):
  ...
  high_risk=kind in {"delete_file","install_package","run_command","deploy_preview","connect_tool"} or action.get("external_effect",False)
