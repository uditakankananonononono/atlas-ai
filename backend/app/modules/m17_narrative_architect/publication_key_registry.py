...
class PublicationKeyRow(Base):
 __tablename__='m17_publication_provider_keys';__table_args__=(UniqueConstraint('tenant_id','provider','key_id'),)
...
class RegisterPublicationKey(BaseModel):provider:str=Field(min_length=1);key_id:str=Field(min_length=1);public_key_base64:str=Field(min_length=1)
class PublicationKeyRegistry:
 ...
 def register(self,b:RegisterPublicationKey):
  ...
  with self.sessions.begin() as db:
   row=db.scalar(select(PublicationKeyRow).where(PublicationKeyRow.tenant_id==self.tenant_id,PublicationKeyRow.provider==b.provider,PublicationKeyRow.key_id==b.key_id))
   if row:
    if row.public_key!=raw:raise ValueError('key_id is already bound to different public key bytes')
    return row
   row=PublicationKeyRow(tenant_id=self.tenant_id,provider=b.provider,key_id=b.key_id,public_key=raw,fingerprint_sha256=hashlib.sha256(raw).hexdigest(),active=True,created_at=datetime.now(timezone.utc),retired_at=None);db.add(row);db.flush();return row
 def retire(self,provider,key_id):
  with self.sessions.begin() as db:
   row=db.scalar(select(PublicationKeyRow).where(PublicationKeyRow.tenant_id==self.tenant_id,PublicationKeyRow.provider==provider,PublicationKeyRow.key_id==key_id))
   if not row:raise ValueError('publication key not found')
