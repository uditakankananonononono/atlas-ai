...
class Vendor(BaseModel):
 id:str=Field(min_length=1);cost:float=Field(ge=0);quality:float=Field(ge=0,le=100);delivery:float=Field(ge=0,le=100);risk:float=Field(ge=0,le=100)
class SupplyNode(BaseModel):
 id:str=Field(min_length=1);kind:Literal["supplier","plant","warehouse","customer"]
class SupplyLane(BaseModel):
 source:str=Field(min_length=1);destination:str=Field(min_length=1);capacity:float=Field(gt=0);unit_cost:float=Field(ge=0);lead_time_days:float=Field(ge=0)
class SupplyTarget(BaseModel):
 destination:str=Field(min_length=1);required_units:float=Field(gt=0);max_lead_time_days:float|None=Field(default=None,gt=0)
class SupplyChainInput(BaseModel):
 nodes:list[SupplyNode]=Field(min_length=2);lanes:list[SupplyLane]=Field(min_length=1);service_targets:list[SupplyTarget]=Field(min_length=1)
 ...
 def validate_network(self):
  ...
  for lane in self.lanes:
   if lane.source not in known or lane.destination not in known:raise ValueError("every lane endpoint must name a supplied node")
   if lane.source==lane.destination:raise ValueError("a supply lane cannot be a self-loop")
  for target in self.service_targets:
   if target.destination not in known:raise ValueError("every service target destination must name a supplied node")
  return self
class DemandInput(BaseModel):
 history:list[float]=Field(min_length=3);horizon:int=Field(default=1,ge=1,le=52);window:int=Field(default=3,ge=2)
 @field_validator("history")
 @classmethod
 def nonnegative_history(cls,v):
  if any(x<0 for x in v):raise ValueError("demand history cannot contain negative values")
  return v
 @model_validator(mode="after")
 def enough_history(self):
  if self.window>=len(self.history):raise ValueError("window must be smaller than history so forecast error can be backtested")
  return self
class InventoryInput(BaseModel):
 annual_demand:float=Field(gt=0);order_cost:float=Field(gt=0);annual_holding_cost_per_unit:float=Field(gt=0);lead_time_days:float=Field(ge=0);daily_demand:float=Field(ge=0);safety_stock:float=Field(default=0,ge=0);current_stock:float|None=Field(default=None,ge=0);on_order:float=Field(default=0,ge=0)
