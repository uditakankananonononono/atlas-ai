...
def parse_rss(xml_text:str,source_id:str,source_name:str,limit:int=25)->list[dict]:
 ...
 for item in root.iter():
  ...
  items.append({'title':title,'url':(fields.get('link','') or fields.get('source_url','')).strip(),'published':_clean(fields.get('pubDate',fields.get('date','')),40),'summary':_clean(fields.get('description',''),500),'feed_id':source_id,'feed_name':fields.get('source',source_name) if source_id=='google_news' else source_name,'feed_homepage':fields.get('source_url','')})
