...
class ProviderKeyRow(Base):
 __tablename__='m12_provider_public_keys';__table_args__=(UniqueConstraint('tenant_id','provider','key_id'),)
...
class RegisterProviderKey(BaseModel):provider:str=Field(min_length=1);key_id:str=Field(min_length=1);public_key_base64:str=Field(min_length=1)
class ProviderKeyRegistry:
 ...
 def register(self,b:RegisterProviderKey):
  ...
  with self.sessions.begin() as db:
   row=db.scalar(select(ProviderKeyRow).where(ProviderKeyRow.tenant_id==self.tenant_id,ProviderKeyRow.provider==b.provider,ProviderKeyRow.key_id==b.key_id))
   if row:
    if row.public_key!=raw:raise ValueError('key_id is already bound to different public key bytes')
    return row
   row=ProviderKeyRow(tenant_id=self.tenant_id,provider=b.provider,key_id=b.key_id,public_key=raw,fingerprint_sha256=hashlib.sha256(raw).hexdigest(),active=True,created_at=datetime.now(timezone.utc),retired_at=None);db.add(row);db.flush();return row
 def retire(self,provider,key_id):
  with self.sessions.begin() as db:
   row=db.scalar(select(ProviderKeyRow).where(ProviderKeyRow.tenant_id==self.tenant_id,ProviderKeyRow.provider==provider,ProviderKeyRow.key_id==key_id))
   if not row:raise ValueError('provider key not found')
