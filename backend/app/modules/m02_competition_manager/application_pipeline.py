...
class ApplicationAnswerPipeline:
 def __init__(self,humanizer,approvals):self.humanizer,self.approvals=humanizer,approvals
 async def prepare_review(self,competition_id,answers,provider='openai'):
  ...
  req=self.approvals.put(ApprovalRequest(id=str(uuid.uuid4()),module_id=2,action_type='review_humanized_application_answers',payload={'competition_id':competition_id,'pipeline':['ai_curated_draft','natural_voice_pass','owner_review','browser_stage','separate_final_submit_approval'],'exact_answers':preview,'answer_sha256':digest,'submission_enabled':False,'humanization_purpose':'clarity_and_natural_voice','prohibitions':['detector_evasion','authorship_misrepresentation','fabricated_content']}))
