...
class IntegratedApplicationFlow:
 def __init__(self,corpus,drafter,humanizer,approvals):self.corpus,self.drafter,self.humanizer,self.approvals=corpus,drafter,humanizer,approvals
 async def prepare(self,competition_id,official_url,fields,provider='openai'):
  ...
  package=await ApplicationAnswerPipeline(self.humanizer,self.approvals).prepare_review(competition_id,exact,provider)
