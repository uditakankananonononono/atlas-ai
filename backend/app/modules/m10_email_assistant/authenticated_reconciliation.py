...
class VerifyReconciliationEvidence(BaseModel):
 ...
 def unique(self):
  ...
  return self
def verify_reconciliation_evidence(body:VerifyReconciliationEvidence,key_lookup)->dict:
 ...
 for a in sorted(body.attestations,key=lambda x:x.reviewer_id):
  ...
  except LookupError as exc:raise ValueError(f'untrusted reviewer key: {exc}') from exc
 ...
 artifact={'messages':messages,'reviewers':reviews};return {'valid':True,**artifact,'evidence_sha256':hashlib.sha256(json.dumps(artifact,sort_keys=True,separators=(',',':')).encode()).hexdigest(),'boundary':'Verifies supplied source-message bytes and reviewer Ed25519 attestations against the tenant reviewer-key registry (active keys only, bound to the reviewer, enrolled with proof-of-possession by that reviewer or an atlas-admin). It does not establish message authorship, persist reconciliation, create tasks, draft, remind, or send.'}
