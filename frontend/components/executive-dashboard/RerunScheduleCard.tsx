...
import type {RerunProposalRow,RerunScheduleCard as Card} from "./api";
...
export default function RerunScheduleCard({card,onOpenApproval}:{card:Card|null;onOpenApproval?:(row:RerunProposalRow)=>void}){
  if(!card)return null;
  ...
  return <section aria-label="Scheduled re-runs" className="rounded-xl bg-slate-900 p-4">
    ...
    {card.overdue.length>0&&<><h3 className="mt-3 text-sm font-semibold text-amber-300">Overdue proposals</h3><ul className="mt-1 space-y-1 text-sm">{card.overdue.map(r=><li key={r.rerun_approval_id}><button type="button" onClick={()=>onOpenApproval?.(r)} disabled={!onOpenApproval} aria-label={`Open approval for re-run of ${r.original_approval_id}`} className="flex w-full justify-between rounded bg-slate-950 p-2 text-left hover:bg-slate-800"><span>Re-run of {r.original_approval_id}</span><span className="text-xs text-slate-400">{stateLabel[r.state]??r.state} · {r.age_hours}h</span></button></li>)}</ul></>}
