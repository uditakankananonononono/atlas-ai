...
| 1 | Opportunity Discovery | RSS/Atom, GitHub and existing Devpost scan path (Devpost endpoint access not live-verified); live spaCy NER + dateparser deadlines + free local embedding match with per-row engine provenance, SQL state, gated digests; separate 17-platform free student directory: 10 live-smoked public listing adapters (RSS/HTML, read-only ranked links) and 7 launch-only platforms, plus 20 tested read-only evidence transforms (deadline/award/delivery/level/region mentions, canonical IDs, dedup, filters and counts), each labeled in [M01 source notes](backend/app/modules/m01_opportunity_discovery/INTEGRATION.md) | Account-based matching/auto-apply/autofill is not integrated and requires authenticated browser workflows plus approval; student directory is not merged into persisted scans; scheduled source fleet, DeBERTa eligibility classifier, pgvector persistence of embeddings, 200 verified scholarship sources |
...
| 13 | Browser Agent | sessions, URL safety, form matching, screenshot-bound single-use approvals, approvals bound to a persisted pre-submit capture with live re-check and one attempt per capture; paired-PC session bridge: daemon on the owner's PC drives her logged-in browser (CDP attach or persistent profile), ed25519-paired devices with per-device capability grants, human-speed pacing, hash-chained action receipts, one-shot HMAC submit tokens verified by the daemon so every submit-class click stays capture-bound and approval-gated, login walls/challenges/rate limits reported as blocked and never evaded | deployed Playwright/VLM runtime and artifact storage; signed daemon installers, OS-keystore identity, native permission prompts, kill switch, and live acceptance on the owner's real accounts |
...
| 19 | Idea Incubator | information-gain assumption burn-down and budget-feasible experiment selection,  budget-capped previews and approval gates; luxury venture studio grounded in real free sources (SEC EDGAR XBRL company facts including IFRS foreign filers, Stooq daily prices, Wikipedia/Wikidata brand records, WWD/Robb Report/LUXUO RSS, Google News RSS, curated $0 report registry) with per-source fetch reporting and auto-extracted review-required signals; 20 phase-2 capabilities: peer comparison (YoY growth, net margin, mixed-currency flag), SEC full-text filing search, SEC registrant contact sheets, topic-tagged news, mention snapshots, evidence refresh diffs, live source health, Van Westendorp pricing, TAM/SAM/SOM sizing, interview scripts, validation-method library, pitch outlines, follow-up plans, objection sheets, KPI sheets, positioning maps, scorecard explainers, sector presets, comparable sets, and markdown digests | durable long-running incubation orchestration; SEC ticker resolution beyond the 15-name watchlist needs a declared operator contact (ATLAS_CONTACT_EMAIL); PurseBlog blocks non-browser clients and Stooq intermittently serves bot checks - reported, never spoofed; McKinsey/BCG free reports are browser-only |
...
| 21 | Claire | paired-device hash-chain result receipt verification,  in-Atlas workflows, bounded capabilities, expiring fingerprinted device pairing/revocation, per-action gates and deception refusal | installable signed paired-PC daemon (an unsigned working daemon now exists in M13's pc_daemon, paired and receipt-verified; signing, OS-keystore identity and durable orchestration remain) |
| 22 | Tools Hub | multi-source free discovery (`sources.py`, verified live): git repositories (GitHub/GitLab/Codeberg), blog sites (DEV.to, WordPress.com Reader, Medium tag RSS), articles (Hacker News Algolia), podcasts (Apple Search API; optional keyed Podcast Index), plus any RSS/Atom feed via a streaming, size-capped parser that marks audio enclosures as podcast episodes; concurrent fan-out with per-source failure isolation, stats and cooldowns (`GET /tools-hub/sources`); kind-filtered + batch discovery, custom scoring weights with per-candidate score explanations, canonical dedup, saved-query diffs and query history, feed autodiscovery/OPML import-export/persistent feed watching, runtime-persisted blocklist, repository evidence refresh, JSON/CSV/markdown export, https-only enforcement; durable tenant-persisted install pipeline (`/tools-hub/pipeline`): pre-scan + content-addressed artifact store, Module 0 approval bound to tool/version/artifact/manifest hashes, worker job that re-verifies and re-scans before the installer consumes a single-use grant, installer-written receipts, persisted portfolio with version history and separately approved rollback; free official GitHub/PyPI/npm discovery with tenant-persisted, de-duplicated candidates; proposals straight from a candidate that fetch the PyPI wheel or npm tarball and check it against the registry-published SHA-256/SHA-512 before scanning; separately approved sandboxed smoke-run (no network, cleared env, read-only system, rlimits) of the installed entrypoint with hashed logs recorded on the portfolio | Bitbucket and Gitee excluded after live verification (no usable unauthenticated search); feed autodiscovery finds only feeds a site actually serves; query history/diffs are in-memory (blocklist and feed-watch cursors persist); DEV.to/Medium discovery is tag-scoped, not full-text; Podcast Index needs an optional free key; blog/podcast/article candidates are research leads only - installs remain registry-digest-gated; GitHub-only candidates cannot be installed (no registry-published digest); PyPI sdist-only packages are refused (Atlas does not run builds); smoke-run proves the entrypoint loads, not that the tool does useful work; no dependency resolution; Python smoke-runs now go through M4's merged sandbox backend (bubblewrap/Docker); Node smoke-runs still use the bundled bubblewrap runner, which needs bwrap in the worker image |
| 23 | Study Abroad | application requirement/essay-claim evidence matrix, advising, identity interview, essay tools and lifecycle planning, 20-source public admitted-case reading shelf opening publisher pages, searchable link index and robots-gated live metadata (no essay copies); 20 student-supplied application-planning checks with editable UI and focused tests | live program-source and application acceptance |
...
- **Paired computer:** signed installers tested on each supported OS, OS-keystore identity, native permission prompts, kill switch, update/uninstall path, and signed action receipts from real machines. The M13 session bridge daemon (docs/paired-pc-bridge.md) is the working unsigned starting point: pairing, capability grants, pacing, receipt verification and approval-bound submits are implemented and tested in-process; real-machine acceptance remains.
...
Atlas uses the model layer shared with Meemee and Sugarcode: https://github.com/uditakankananonononono/shared-models, vendored at `backend/instinct_models` (pin in `backend/instinct_models/VENDORED.md`, currently 8e8eda6; re-sync with `scripts/sync_shared_models.sh <sha>`).
...
- Any module can use the shared layer through `app.core.providers.generate(prompt, "shared")` (private: hosted route skipped) or `"shared-public"` (may use the HF router, free tier, metered past it). M07 contract extraction uses it with private=True; M14 planning now defaults to `"shared"` instead of `"openai"`; M12 lists `shared:instinct` at zero cost and routes only free models unless `ATLAS_ALLOW_PAID=true` (paid model ids are also refused at call time).
...
- Jev (TypeSafe AI's hosted "System One" evaluation model) is available through `app.core.shared_model_layer.jev_eval()`: it evaluates one state against typed questions (choice / score / noul) and returns structured decisions with probabilities. It is NOT a chat model, it is key-gated and paid (credits, https://thejevai.com/pricing - no free tier as of 2026-09-26), and it is OFF unless a key is set (`ATLAS_JEV_API_KEY`, `INSTINCT_JEV_API_KEY` or `JEV_API_KEY`; create one at https://thejevai.com/settings/apikeys). Hosted route: never send it private state.
