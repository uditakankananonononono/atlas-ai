...
- Jev (TypeSafe AI's hosted "System One" evaluation model) is available through `app.core.shared_model_layer.jev_eval()`: it evaluates one state against typed questions (choice / score / noul) and returns structured decisions with probabilities. It is NOT a chat model, it is key-gated and paid (credits, https://docs.typesafe.ai/models - no free tier as of 2026-09-26), and it is OFF unless a key is set (`ATLAS_JEV_API_KEY`, `INSTINCT_JEV_API_KEY` or `JEV_API_KEY`; create one at https://console.typesafe.ai/keys). Hosted route: never send it private state.
